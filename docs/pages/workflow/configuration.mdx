...
The app config (**app.json**, **app.config.js**, **app.config.ts**) is used for configuring how a project loads in [Expo Go](/get-started/expo-go/), [Expo Prebuild](/workflow/prebuild) generation, and the OTA update manifest. You can think of this as an `index.html` but for React Native apps.
...
- Warning: `eas-cli` will not be able to automatically update a standalone dynamic config with the Expo projectId property. If your dynamic config extends a static config, `eas-cli` will update the static config.
